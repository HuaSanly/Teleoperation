...
void logCudaFailureOnce(const CudaYuv422ConverterResult &result,
                        int src_fd,
                        uint32_t width,
                        uint32_t height)
{
...
bool VideoConverter::initialize(const Config &config)
{
    ...
    out_params.layout = NVBUF_LAYOUT_PITCH;
    ...
    RCLCPP_INFO(rclcpp::get_logger("teleop_robot_bridge.video"),
                "VideoConverter: CUDA-only YUV422->NV12 in=%ux%u out=%ux%u layout=pitch pool=%u",
    ...
    for (size_t i = 0; i < config_.buffer_pool_size; ++i)
    {
        ...
        if (NvBufSurfaceCreate(&surf, 1, &out_params) != 0 || !surf)
        {
            std::cerr << "VideoConverter: failed to create CUDA NV12 surface " << i << std::endl;
            destroyBuffers();
            return false;
        }
        surf->numFilled = 1;
    ...
    for (void *surface : surfaces_)
    {
        ...
        if (!dst || !cuda_converter_->prepareOutput(dst, &cuda_result))
        {
            const int fd = dst ? dst->surfaceList[0].bufferDesc : -1;
            logCudaFailureOnce(cuda_result, fd, config_.width, config_.height);
            RCLCPP_ERROR(rclcpp::get_logger("teleop_robot_bridge.video"),
                         "VideoConverter: failed to prepare CUDA output pool");
            destroyBuffers();
            return false;
        }
    }
                                   ...
                                   int &output_fd_out)
{
    ...
    if (yuv_dmabuf_fd < 0 || dmabuf_fds_.empty() || !cuda_converter_)
    {
        return false;
    }
    ...
    if (!logged_surface_details_.exchange(true, std::memory_order_relaxed))
    {
        ...
        RCLCPP_INFO(rclcpp::get_logger("teleop_robot_bridge.video"),
                    "VideoConverter: first map wait=%ldus call=%ldus output_format=nv12",
    ...
    const bool ok = cuda_converter_->convert(src_surf, dst, &cuda_result);
    ...
    stats_transform_us_total_.fetch_add(call_us);
    ...
    output_fd_out = dmabuf_fds_[out_idx];
    return true;
}
