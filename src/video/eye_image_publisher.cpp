...
bool EyeImagePublisher::initialize(rclcpp::Node & nh, uint32_t src_width, uint32_t src_height, const Config& config)
{
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        {
            RCLCPP_ERROR(rclcpp::get_logger("teleop_robot_bridge.video"), "EyeImagePublisher: Failed to create NV12 surface %d", i);
            destroyBuffers();
            return false;
        }
    ...
    RCLCPP_INFO(rclcpp::get_logger("teleop_robot_bridge.video"), "EyeImagePublisher initialized (shared decoder mode): src=%ux%u, out=%ux%u, topics=[%s, %s]",
...
void EyeImagePublisher::processFrame(NvBufSurface* src_surface, uint64_t timestamp_us)
{
    ...
    if (n == 0 || (n % 300 == 0))
    {
        ...
        RCLCPP_INFO(rclcpp::get_logger("teleop_robot_bridge.video"), "EyeImagePublisher (shared decoder): frame %lu, process=%.2fms, subs=[L:%zu, R:%zu]",
...
bool EyeImagePublisher::cropScaleToNV12(NvBufSurface* src_surf, int dst_idx, bool is_right_eye)
{
    ...
    if (!src_surf)
    {
        RCLCPP_ERROR(rclcpp::get_logger("teleop_robot_bridge.video"), "EyeImagePublisher: src_surf is null");
        return false;
    }
    ...
    if (ret != 0)
    {
        RCLCPP_ERROR(rclcpp::get_logger("teleop_robot_bridge.video"), "EyeImagePublisher: NvBufSurfTransform failed ret=%d (is_right=%d)", ret, is_right_eye);
        return false;
    }
...
bool EyeImagePublisher::convertNV12ToRGB(NvBufSurface* surf, uint8_t* rgb_out, size_t rgb_size)
{
    ...
    if (!surf)
    {
        RCLCPP_ERROR(rclcpp::get_logger("teleop_robot_bridge.video"), "EyeImagePublisher: convertNV12ToRGB surf is null");
        return false;
    }
    ...
    {
        RCLCPP_ERROR(rclcpp::get_logger("teleop_robot_bridge.video"), "EyeImagePublisher: NvBufSurfaceMap failed");
        return false;
    }
