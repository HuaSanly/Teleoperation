...
#ifndef TRB_HAS_GSTREAMER_AUDIO
#define TRB_HAS_GSTREAMER_AUDIO 0
#endif

#if TRB_HAS_GSTREAMER_AUDIO
...
namespace trb::audio
{
    ...
    namespace
    {
        ...
        bool ensureGstInitialized(const rclcpp::Logger &logger)
        {
            ...
            if (!gst_init_check(nullptr, nullptr, &error))
            {
                ...
                RCLCPP_ERROR(logger, "GStreamer init failed: %s", message);
                if (error)
                {
                    g_error_free(error);
                }
    ...
    {
        explicit Impl(rclcpp::Logger logger_in)
            : logger(std::move(logger_in)), throttle_clock(std::make_shared<rclcpp::Clock>(RCL_STEADY_TIME))
        {
        }

        rclcpp::Logger logger;
        rclcpp::Clock::SharedPtr throttle_clock;
        Config config;
        ...
        void maybeLogStatsLocked(const std::chrono::steady_clock::time_point &now)
        {
            ...
            RCLCPP_INFO(logger,
                        "[AUDIO] cap=%.1fpps up=%.1fpps %.1fkbps down=%.1fpps %.1fkbps push_fail=%lu running=%d",
                        capture_pps,
                        uplink_pps,
                        uplink_kbps,
                        downlink_pps,
                        ...
                        running.load(std::memory_order_acquire) ? 1 : 0);
        ...
        {
            if (!running.load(std::memory_order_acquire) || !capture_active.load(std::memory_order_acquire))
            {
            ...
            const uint64_t pts_ns = GST_BUFFER_PTS_IS_VALID(buffer) ? GST_BUFFER_PTS(buffer) : GST_CLOCK_TIME_NONE;
        ...
        static void logPipelineMessages(const rclcpp::Logger &logger,
                                        GstElement *pipeline,
                                        ...
                                        const std::string &description)
        {
            ...
            RCLCPP_WARN(logger, "%s description: %s", pipeline_name, description.c_str());
            while (true)
            {
        ...
        std::string buildPlaybackPipelineDescription() const
        {
            ...
            pipeline << "appsrc name=audio_playback_src is-live=true format=time block=false do-timestamp=false"
                     ...
                     << " ! audio/x-raw,format=S16LE,rate=" << config.sample_rate
                     << ",channels=" << config.channels
                     << " ! volume volume=" << config.playback_volume;
        ...
        bool createCapturePipeline()
        {
            ...
            RCLCPP_INFO(logger, "Audio capture pipeline: %s", capture_pipeline_description.c_str());
            ...
            if (!capture_pipeline)
            {
                ...
                RCLCPP_ERROR(logger, "Failed to create audio capture pipeline: %s", message);
                if (error)
                {
                    g_error_free(error);
                }
            ...
            if (error)
            {
                RCLCPP_WARN(logger, "Audio capture pipeline warning: %s", error->message);
                g_error_free(error);
            }
            ...
            if (!capture_sink)
            {
                RCLCPP_ERROR(logger, "Failed to get appsink from audio capture pipeline");
                return false;
            }
        ...
        bool createPlaybackPipeline()
        {
            ...
            RCLCPP_INFO(logger, "Audio playback pipeline: %s", playback_pipeline_description.c_str());
            ...
            if (!playback_pipeline)
            {
                ...
                RCLCPP_ERROR(logger, "Failed to create audio playback pipeline: %s", message);
                if (error)
                {
                    g_error_free(error);
                }
            ...
            if (error)
            {
                RCLCPP_WARN(logger, "Audio playback pipeline warning: %s", error->message);
                g_error_free(error);
            }
            ...
            if (!playback_src)
            {
                RCLCPP_ERROR(logger, "Failed to get appsrc from audio playback pipeline");
                return false;
            }
        ...
        {
            config = cfg;
            if (!ensureGstInitialized(logger))
            {
                return false;
            }
        ...
        bool startPipelines()
        {
            ...
            if (capture_pipeline)
            {
                ...
                {
                    RCLCPP_WARN(logger, "Failed to start audio capture pipeline, continuing without uplink audio");
                    logPipelineMessages(logger, capture_pipeline, "audio capture pipeline", capture_pipeline_description);
                    gst_element_set_state(capture_pipeline, GST_STATE_NULL);
                    unrefAppSink(capture_sink);
                    unrefElement(capture_pipeline);
                }
            ...
            if (playback_pipeline)
            {
                ...
                if (ret == GST_STATE_CHANGE_FAILURE)
                {
                    RCLCPP_WARN(logger, "Failed to start audio playback pipeline, continuing without downlink audio");
                    logPipelineMessages(logger, playback_pipeline, "audio playback pipeline", playback_pipeline_description);
                    gst_element_set_state(playback_pipeline, GST_STATE_NULL);
                    unrefAppSrc(playback_src);
                    unrefElement(playback_pipeline);
                }
            ...
            RCLCPP_INFO(logger,
                        "Audio pipelines started: uplink=%d downlink=%d",
                        capture_active.load(std::memory_order_acquire) ? 1 : 0,
                        playback_active.load(std::memory_order_acquire) ? 1 : 0);
            return true;
        }
        ...
        bool pushRemotePacket(const uint8_t *data, size_t size, uint64_t timestamp_ntp)
        {
            if (!running.load(std::memory_order_acquire) ||
                !playback_active.load(std::memory_order_acquire) ||
                !playback_src ||
                !data ||
                size == 0)
            {
            ...
            if (ret != GST_FLOW_OK)
            {
                ...
                RCLCPP_WARN_THROTTLE(logger,
                                     *throttle_clock,
                                     1000,
                                     "Failed to push remote Opus packet into playback pipeline: %d",
                                     ret);
                return false;
            }
    ...
    AudioManager::AudioManager(rclcpp::Logger logger)
        : impl_(new Impl(std::move(logger)))
    {
    }
    ...
    bool AudioManager::initialize(const Config &config)
    {
        impl_->config = config;
#if TRB_HAS_GSTREAMER_AUDIO
        return impl_->initializePipelines(config);
#else
        RCLCPP_WARN(impl_->logger,
                    "AudioManager built without GStreamer audio support; install gstreamer-1.0 and gstreamer-app-1.0 dev packages to enable audio");
        return false;
#endif
    }
    ...
    void AudioManager::resetPlaybackTimeline()
    {
#if TRB_HAS_GSTREAMER_AUDIO
        impl_->resetPlaybackTimeline();
#endif
    }

    const AudioManager::Config &AudioManager::config() const
    {
        return impl_->config;
    }
