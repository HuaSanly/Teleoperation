...
class Metaclass_WaistJointFeedback(type):
    """Metaclass of message 'WaistJointFeedback'."""
    ...
    def __import_type_support__(cls):
        try:
            ...
            logger = logging.getLogger(
                'teleop_robot_bridge.msg.WaistJointFeedback')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
            cls._CREATE_ROS_MESSAGE = module.create_ros_message_msg__msg__waist_joint_feedback
            cls._CONVERT_FROM_PY = module.convert_from_py_msg__msg__waist_joint_feedback
            cls._CONVERT_TO_PY = module.convert_to_py_msg__msg__waist_joint_feedback
            cls._TYPE_SUPPORT = module.type_support_msg__msg__waist_joint_feedback
            cls._DESTROY_ROS_MESSAGE = module.destroy_ros_message_msg__msg__waist_joint_feedback
...
class WaistJointFeedback(metaclass=Metaclass_WaistJointFeedback):
    """Message class 'WaistJointFeedback'."""
    ...
    def __init__(self, **kwargs):
        ...
        if 'effort' not in kwargs:
            self.effort = numpy.zeros(3, dtype=numpy.float64)
        else:
    ...
    def effort(self, value):
        if self._check_fields:
            if isinstance(value, numpy.ndarray):
                ...
                assert value.size == 3, \
                    "The 'effort' numpy.ndarray() must have a size of 3"
                self._effort = value
                return
            from collections.abc import Sequence
            from collections.abc import Set
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 all(not (val < -1.7976931348623157e+308 or val > 1.7976931348623157e+308) or math.isinf(val) for val in value)), \
                "The 'effort' field must be a set or sequence with length 3 and each value of type 'float' and each double in [-.000000, .000000]"
